(function(window, document) {
	window['Copenhagen'] = {
		...
		initSelectorAll: function(selector) {
			return [].slice.call(document.querySelectorAll(selector)).map(function(el) {
				...
				var cfg = attrs
					.filter(function(a) {
						return a.nodeName.startsWith('data-');
					})
	...
	function Control() {
		...
		this._enabled = this.hasOwnProperty('_enabled') ? this._enabled : true;
	...
	Control.prototype._mapEvent = function(event) {
		return isMobile() ? this._mobileEventMap[event] || event : event;
	};
	...
	Control.prototype.listen = function() {
		...
		Object.keys(events).forEach(function(elementName) {
			...
			Object.keys(events[elementName]).forEach(function(eventName) {
				...
				var wrapperFn = disabled
					? function() {
							!self._enabled && fn.apply(self, arguments);
					  }.bind(self)
					: function() {
							self._enabled && fn.apply(self, arguments);
					  }.bind(self);
		...
		Object.keys(controlActions).forEach(function(ctrlName) {
			...
			Object.keys(actions).forEach(function(actionName) {
				...
				var wrapperFn = disabled
					? function() {
							!control._enabled && fn.apply(self, arguments);
					  }.bind(self)
					: function() {
							control._enabled && fn.apply(self, arguments);
					  }.bind(self);
		...
		Object.keys(selfActions).forEach(function(actionName) {
			...
			var wrapperFn = disabled
				? function() {
						!self._enabled && fn.apply(this, arguments);
				  }.bind(self)
				: function() {
						self._enabled && fn.apply(self, arguments);
				  }.bind(self);
	...
	Control.prototype.selectorAll = function(selector) {
		var elements = this.elements;
		return [].slice.call(elements.main.querySelectorAll(selector)).filter(function(el) {
			while (el.parentNode) {
	...
	Control.prototype.createControls = function(elements) {
		return [].slice.call(elements.main.querySelectorAll('control')).reduce(
			function(controls, el) {
				...
				if (fromName && !controlName) {
					if (el.getAttributeNames().length > 1) {
						throw new Error(
							'If "name" is specified without "control" on <control> element, loading from .addControl call. Can not specify additional configuration.',
						);
					}
	...
	Control.prototype.keyboardShortcut = function(e) {
		...
		if (modifierKey) {
			...
			var command = [altKey ? 'alt' : '', shiftKey ? 'shift' : '', keyChar ? keyChar : '']
				.filter(function(v) {
					return v;
				})
				.join(' ');
	...
	function CPHEditor(app, cfg) {
		...
		this._history = {
			initialValue: cfg.value === undefined ? '' : cfg.value.replace(/\r/gi, '') + '', // remove carriage returns
			userActions: [],
		};
		...
		this.hotkeys = Object.keys(this.constructor.prototype.hotkeys).reduce(
	...
	CPHEditor.prototype.formatters = {
		...
		markdown: function(line, inString, inComment) {
			...
			return inString ? '<span class="hljs-code">' + formatted + '</span>' : formatted;
		},
		...
		mousemove: function capture(e) {
			if (this._verticalScrollerGrab) {
				...
				if (e.buttons !== 1) {
					...
					this._verticalScrollerTimeout = setTimeout(
						function() {
				...
				if (e.buttons !== 1) {
					...
					this._horizontalScrollerTimeout = setTimeout(
						function() {
		...
		'find-replace': {
			...
			replace: function(ctrl, value, replaceValue, isCaseSensitive) {
				if (this._find.currentIndex !== -1) {
					this.user.resetCursor();
					this.user.cursors[0].select(
						this._find.currentIndex,
						this._find.currentIndex + this._find.currentValue.length,
					);
		...
		textarea: {
			...
			contextmenu: function capture(e) {
				...
				if (!e.defaultPrevented) {
					e.preventDefault();
					this._contextMenu = new CPHContextMenu(this.app, {
						parent: this,
						items: [
							{
								icon: 'rotate-ccw',
			...
			cut: function(e) {
				...
				e.clipboardData.setData('text/plain', this.value.slice(cursor.selectionStart, cursor.selectionEnd));
			...
			keydown: function capture(e) {
				...
				var hotkey = [['', 'ctrl'][ctrlKey | 0], ['', 'alt'][e.altKey | 0], ['', 'shift'][e.shiftKey | 0], key]
					.filter(function(v) {
						return !!v;
					})
					.join('+');
				...
				if (!key) {
					...
					if (key === 'escape') {
						...
						preventDefaultAndStopPropagation();
						if (this._autocomplete) {
							this.dispatch('autocomplete/submit', this, this._autocomplete.name, this._autocomplete.result);
						} else {
							...
							if (this._autocomplete) {
								...
								if (this._suggestion) {
									...
									this.userAction(
										...
										this._suggestion.adjust,
										this._suggestion.cursorLength,
									);
								} else {
						...
						if (this._autocomplete && (direction === 'up' || direction === 'down')) {
							this.dispatch('autocomplete/' + direction, this, this._autocomplete.name, this._autocomplete.result);
						} else {
						...
						this.scrollToText();
					} else if (revComplement && this.value[this.user.cursors[0].selectionStart - 1] === revComplement) {
						preventDefaultAndStopPropagation();
						...
						this.scrollToText();
					} else if (fwdComplement && this.value[this.user.cursors[0].selectionStart] !== fwdComplement) {
						preventDefaultAndStopPropagation();
	...
	CPHEditor.prototype.gotoHistory = function(amount) {
		...
		if (historyIndex !== this._historyIndex) {
			...
			if (this._historyIndex === -1) {
				...
				this.user.getSortedCursors().forEach(
					function(cursor) {
						cursor.clamp(this.value);
					}.bind(this),
				);
	...
	CPHEditor.prototype.userAction = function(name) {
		if (this.isReadOnly() || !this.isEnabled()) {
			...
			var args = [].slice.call(arguments, 1);
			return this._userAction(name, args, this.getActiveLanguageDictionary(), this.value, false);
		}
	};
	...
	CPHEditor.prototype._userAction = function(name, args, lang, value, replay) {
		...
		if (!replay) {
			...
			this._history.userActions.push({
				value: value,
				cursors: this.user.cursors.map(function(cursor) {
					return cursor.clone();
				}),
		...
		var bigCursor = new CPHCursor(
			sortedCursors[0].selectionStart,
			sortedCursors[sortedCursors.length - 1].selectionEnd,
		);
		...
		if (name === 'RemoveText' && parseInt(args[0]) < 0) {
			// catch backspaces
		...
		if (!replay) {
			this.user.getSortedCursors().forEach(function(cursor) {
				cursor.clamp(value);
			});
				...
				function() {
					if (this._renderQueue.length) {
						...
						var scrollValue = [
							...
							Math.max(0, this.inputElement.scrollTop + this.fixScrollTop),
						].join(',');
						var viewportValue = [this.width, this.height].join(',');
	...
	CPHEditor.prototype.__renderSelection = function(force) {
		...
		if (
			...
			this.inputElement.setSelectionRange(
				selectionStart,
				selectionEnd,
				cursor.position >= cursor.pivot ? 'forward' : 'backward',
			);
		}
	};
	...
	CPHEditor.prototype.__setScroll = function(scrollTop, scrollLeft) {
		if (
			scrollTop > -1 &&
			Math.min(scrollTop, this.inputElement.scrollHeight - this.inputElement.offsetHeight) !==
		...
		if (
			scrollLeft > -1 &&
			Math.min(scrollLeft, this.inputElement.scrollWidth - this.inputElement.offsetWidth) !==
				this.inputElement.scrollLeft
		) {
	...
	CPHEditor.prototype.__render = function(
		...
		// HACK: fixScrollTop is for Firefox
		if (this.fixScrollTop !== -1 && this.fixScrollTop !== this.inputElement.scrollTop) {
		...
		// Set max line
		if (valueChanged) {
			this._maxLine = lines.reduce(function(cur, line) {
				return [line, cur][(line.length < cur.length) | 0];
			}, '');
		}
		...
		var reduceFrames = function(frames) {
			frames = frames.map(function(f) {
				return Math.max(0, Math.min(f, lines.length - 1));
			});
		...
		if (frameIndex !== this.virtualFrameIndex) {
			this.inputElement.value = frameCount > frames.length ? frameValue + '\n' + this._maxLine : frameValue;
		...
		if (
			...
			for (var i = 0; i < renderLines.length; i++) {
				...
				var lineError =
					!this._autocomplete && this._errorPos.lineIndex === renderStartLineIndex + i && this._errorPos.enabled
						? this._errorPos
						: null;
				...
				for (var ci = 0; ci < cursors.length; ci++) {
					...
					if (unbound) {
						...
						selected = true;
						selectionPts.push([
							c.selectionStart - renderCursorOffset,
							c.selectionEnd - renderCursorOffset,
							'lb rb',
							c.direction(),
						]);
						if (!c.width()) {
				...
				var formattedLine = this.format(
					...
					complements.slice(),
					lineError,
					this._find.value ? this._findRE : null,
				);
		...
		var scrollerHeight = Math.min(height, Math.max(this._minScrollerSize, (height / maxHeight) * height));
		var scrollerTranslateY = (top / (maxHeight - height)) * (height - scrollerHeight);
		if (scrollerHeight < height && scrollValueChanged) {
			this.verticalScrollerElement.classList.add('scrolling');
			clearTimeout(this._verticalScrollerTimeout);
			if (!this._verticalScrollerGrab) {
				this._verticalScrollerTimeout = setTimeout(
		...
		var scrollerWidth = Math.min(width, Math.max(this._minScrollerSize, (width / maxWidth) * width));
		var scrollerTranslateX = (left / (maxWidth - width)) * (width - scrollerWidth);
		if (scrollerWidth < width && scrollValueChanged) {
			this.horizontalScrollerElement.classList.add('scrolling');
			clearTimeout(this._horizontalScrollerTimeout);
			if (!this._horizontalScrollerGrab) {
				this._horizontalScrollerTimeout = setTimeout(
	...
	CPHEditor.prototype.format = function(
		...
		var cacheLine = JSON.stringify(
			[].slice.call(arguments, 1, 8).concat((findRE || '').toString(), inComment, inString),
		);
		if (this._formatCache[lineNumber] && this._formatCache[lineNumber][0] === cacheLine) {
			...
			var complementLine = complements.reduce(
				function(complementLine, n) {
					if (n >= offset && n < offset + value.length) {
						var i = n - offset;
						complementLine.line +=
							safeHTML(line.slice(complementLine.index, i)) +
							'<span class="underline' +
							(complements[1] === -1 ? ' no-match' : '') +
							'">' +
							safeHTML(line[i]) +
							'</span>';
			...
			this._formatCache[lineNumber] = [
				...
				'<div class="display">' +
					formatted.replace(/^(\t| )+/gi, function($0) {
						return (
							'<span class="whitespace">' + $0.replace(/\t/gi, '&rarr;&nbsp;').replace(/ /gi, '&middot;') + '</span>'
						);
					}) +
					(suggestion
						? '<span class="suggestion">' +
						  safeHTML(suggestion.value.replace(/\t/gi, '&rarr; '))
								.replace(/ /gi, '&middot;')
								.replace(/\n/gi, '↵') +
						  '</span>'
						: '') +
					'</div>' +
					(complementLine ? '<div class="complement">' + complementLine + '</div>' : '') +
					(lineError
						? '<div class="line-error">' +
						  safeHTML(line.slice(0, lineError.column)) +
						  '<span class="underline">' +
						  safeHTML(line[lineError.column] || '') +
						  '</span></div>'
						: '') +
						  ...
						  selectionPts.reduce(
								function(acc, pt) {
									acc.str =
										acc.str +
										'<span class="spacer">' +
										safeHTML(value.slice(acc.index, pt[0])) +
										'</span>' +
										'<span class="border ' +
										pt[2] +
										' ' +
					...
					(findRE
						? '<div class="find">' +
						  safeHTML(line.replace(findRE, '~~~[~~~$&~~~]~~~')).replace(
								/~~~\[~~~(.*?)~~~\]~~~/gi,
	...
	CPHEditor.prototype.scrollToText = function() {
		...
		this.paddingLeft =
			this.paddingLeft ||
			parseInt(window.getComputedStyle(this.inputElement, null).getPropertyValue('padding-left')) ||
			0;
		this.paddingTop =
			this.paddingTop ||
			parseInt(window.getComputedStyle(this.inputElement, null).getPropertyValue('padding-top')) ||
			0;
		this.paddingBottom =
			this.paddingBottom ||
			parseInt(window.getComputedStyle(this.inputElement, null).getPropertyValue('padding-bottom')) ||
			0;
		...
		var widthLines = this.value
			.slice(this.value.slice(0, cursor.selectionStart).lastIndexOf('\n') + 1, cursor.selectionEnd)
			.split('\n');
		var maxLine = widthLines.reduce(function(maxLine, line) {
			return maxLine.length > line.length ? maxLine : line;
		}, '');
	...
	CPHEditor.prototype.__populateStringLookup = function(callback) {
					...
					.map(function(c) {
						if (!foundString[c]) {
							...
							var start = c
								...
								.join('');
							var end = (lang.stringComplements[c] || lang.multiLineStrings[c])
								.split('')
								.map(function(c) {
									return '\\' + c;
								})
		...
		this._blockLookup = value.replace(blockRE, function($0) {
			return commentChar.repeat($0.length);
		});
	...
	CPHEditor.prototype.inBlock = function(n) {
		...
		return this._blockLookup[n] === c || (n === this.value.length && this._blockLookup[n - 1] === c);
	};
	...
	CPHEditor.prototype.inComment = function(n) {
		var c = String.fromCharCode(0);
		return this._commentLookup[n] === c || (n === this.value.length && this._commentLookup[n - 1] === c);
	};
	...
	CPHEditor.prototype.inString = function(n) {
		var lang = this.getActiveLanguageDictionary();
		return (
			lang.stringComplements[this._commentLookup[n]] ||
			(n === this.value.length && lang.stringComplements[this._commentLookup[n - 1]]) ||
			false
		);
	};
	...
	CPHEditor.prototype.findComplements = function(value, n) {
		...
		var lang = this.getActiveLanguageDictionary();
		if (!str && !lang.forwardComplements[value[n]] && !lang.reverseComplements[value[n]]) {
			n = n - 1;
			str = this.inString(n);
		}
		var len = value.length;
	...
	CPHEditor.prototype.animateNo = function() {
		...
		this._animateNoTimeout = setTimeout(
			function() {
	...
	function CPHContextMenu(app, cfg) {
		...
		this.constructor.activeMenu && this.constructor.activeMenu.close();
		...
		itemEls.forEach(
			function(el, i) {
				...
				hidden = typeof hidden === 'function' ? !!hidden.call(this.parent, this.data) : !!hidden;
				if (hidden) {
	...
	CPHContextMenu.prototype.open = function(e, rightAlign) {
		...
		if (this._boundElement) {
			this.element().style[['marginTop', 'marginBottom'][!!this.element().style.bottom | 0]] = '-4px';

			this.element().style[
				[
					['border-top-left-radius', 'border-top-right-radius'][!this.element().style.left | 0],
					['border-bottom-left-radius', 'border-bottom-right-radius'][!this.element().style.left | 0],
	...
	CPHCursor.prototype.calculateInsertLines = function(value, args) {
		...
		var selectRelative = [-sel.linesPrefix.length, sel.linesSuffix.length + insertValue.length - replaceValue.length];
		...
		var lastLineSuffix = sel.lines[sel.lines.length - 1].slice(
			sel.lines[sel.lines.length - 1].length - sel.linesSuffix.length,
		);
	...
	CPHUser.prototype.loadCursors = function(cursors) {
		this.cursors = cursors.map(function(cursor) {
			return cursor.clone();
		});
	};
	...
	CPHUser.prototype.createNextCursor = function(value) {
		...
		if (matchValue.length) {
			...
			if (cursors.length === len) {
				...
				if (
					index > -1 &&
					this.cursors.filter(function(cursor) {
						return cursor.selectionStart === index;
					}).length === 0
				) {
	...
	CPHUser.prototype.moveCursors = function(value, direction, amount, expandSelection, createCursor) {
		amount = amount === undefined || amount === null ? 1 : parseInt(amount) || 0;
		if (direction === 'left' || direction === 'right') {
			amount = { left: -amount, right: amount }[direction];
			this.getSortedCursors().forEach(
				function(cursor) {
					if (expandSelection) {
			...
			(createCursor ? this.cursors.slice(0, 1) : this.cursors).forEach(
				function(cursor) {
					if (cursor.position !== 0) {
						...
						if (last2n > -1) {
							...
							expandSelection
								...
								: createCursor
								? this.createCursor({
										selectionStart: last2n + 1 + Math.min(offset, prevline.length),
					...
					if (cursor.position !== value.length) {
						...
						if (nextn > -1) {
							...
							expandSelection
								...
								: createCursor
								? this.createCursor({
										selectionStart: nextn + 1 + Math.min(offset, nextline.length),
										offset: cursor.offset,
								  })
	...
	Template.prototype.render = function(control, it) {
		return this._render.call(control, it);
	};
	Template._templates = {};
	Template.find = function(Control) {
		var template = Template._templates[Control.name];
		if (!template) {
			throw new Error('Could not find template: "' + name + '"');
		}
		return template;
	};
	Template.add = function(Control, fn) {
		Template._templates[Control.name] = new Template(fn);
	};
	...
	CodeCompleter.prototype.generateSuggestions = function() {
		...
		return Object.keys(suggestionMap).reduce(
			function(suggestions, language) {
				var phraseList = suggestionMap[language].map(
					function(value) {
						...
						var adjust = cursorStart === -1 ? 0 : cursorStart - value.length + 1;
						if (adjust) {
	...
	CodeCompleter.prototype.complete = function(tree, value, index, subs, inWildcard) {
		...
		return results
			.sort(function(p1, p2) {
				return p2.offset - p1.offset || p1.ranking - p2.ranking;
			})
			.filter(function(p) {
				return p.offset < p.value.length;
			});
	};
...
*/
	...
	Template.add(CPHEditor, function anonymous(it) {
		var out = '<div class="editor" ';
		if (this.debug) {
			out += 'data-debug';
		}
		out +=
			' > <control control="CPHFindReplace" name="find-replace"></control> <div class="read-only"> read only <span data-language></span> </div> <div class="line-container"> <div class="line-numbers"></div> </div> <div class="edit-text"> <div class="scrollbar vertical"><div class="scroller"></div></div> <div class="scrollbar horizontal"><div class="scroller"></div></div> <textarea spellcheck="false" autocomplete="off" autocorrect="off" autocapitalize="off" wrap="off" tabIndex="-1"></textarea> <div class="render"></div> <div class="render sample"><div class="line"><span class="fill">x</span></div></div> <div class="render limit"><span class="fill">' +
			'W'.repeat(80) +
			'</span></div> </div></div>';
		return out;
	});
		...
		if (arr1) {
			...
			while (i < l1) {
				...
				if (item === '-') {
					...
					out += ' <a class="item" data-item-index="' + i + '" ';
					if (item.href) {
						out += 'href="' + item.href + '"';
					}
					out += ' ';
					...
					if (item.icon) {
						out +=
							...
							(item.icon.startsWith('<')
								? item.icon
								: feather.icons[
										(typeof item.icon === 'function' ? item.icon(this.data) : item.icon) || 'chevron-right'
								  ].toSvg()) +
							' ';
					} else {
						out += ' <svg class="empty"></svg> ';
					}
					out += ' ' + (typeof item.title === 'function' ? item.title(this.data) : item.title) + ' </span> ';
					if (item.shortcut) {
						out +=
							' <span class="shortcut"> ' +
							this.generateShortcut(typeof item.shortcut === 'function' ? item.shortcut(this.data) : item.shortcut) +
							' </span> ';
					}
					out += ' </a> ';
				}
	...
	Template.add(CPHFindReplace, function anonymous(it) {
		var out =
			'<div> <div class="row"> <a name="regex" class="btn" tabIndex="0">' +
			regexicon() +
			'</a> <a name="case" class="btn" tabIndex="0">' +
			caseicon() +
			'</a> <input name="find" type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Find in text..."> <div class="pos" data-position>0 / 0</div> <div class="spacer"></div> <a name="prev" class="btn" tabIndex="0">' +
			feather.icons['chevron-up'].toSvg() +
			'</a> <a name="next" class="btn" tabIndex="0">' +
			feather.icons['chevron-down'].toSvg() +
			'</a> <a name="close" class="btn" tabIndex="0">' +
			feather.icons['x'].toSvg() +
			'</a> </div> <div class="row"> <a class="btn" disabled><svg></svg></a> <a class="btn" disabled><svg></svg></a> <input name="replace" type="text" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" placeholder="Replace with..."> <a name="replace-one" class="btn" tabIndex="0"><span>Replace</span></a> <a name="replace-all" class="btn" tabIndex="0"><span>Replace All</span></a> <div class="spacer"></div> </div></div>';
		return out;
	});
})(window, document);
